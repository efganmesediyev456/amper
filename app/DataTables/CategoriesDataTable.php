    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($user) {
                ...
                $edit = '<a href="'.route('admin.categories.edit', $user->id).'" class="btn  btn-sm fs-5"><i class="fas fa-edit"></i></a>';
                $delete = '<form action="' . route('admin.categories.destroy', $user->id) . '" method="POST" class="delete-form">
                                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button type="submit" class="btn btn-sm fs-5 text-danger delete-btn"><i class="fas fa-trash"></i></button>
                            </form>';
                            $subcategories='<a href="'.route('admin.categories.subcategories.index', $user->id).'">Alt kateqoriyalar</a>';
                $html.= $edit.$subcategories.$delete;
                $html.='</div>';
                return $html;
            })
