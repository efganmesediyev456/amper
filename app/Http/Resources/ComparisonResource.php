...
class ComparisonResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $comparisonData = $this->getComparisonData($categoryId);
     ...
     * @return array
     */
    private function getComparisonData(int $categoryId): array
    {
        ...
        $categoryProducts = auth()->user()->comparisons()
            ->with(['product.subProperties.property'])
            ->whereHas('product', function ($query) use ($categoryId) {
                $query->where('category_id', $categoryId);
            })
            ->where('product_id', '!=', $this->id) // Exclude current product
            ->get()
            ->pluck('product');
